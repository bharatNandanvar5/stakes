...
import { Send, Eraser, Clock, Pencil, Trash2, CheckCircle2 } from 'lucide-react';
...
const ScribbleBoard: React.FC = () => {
  ...
  const onMouseMove = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    drawLine(lastPos.current.x, lastPos.current.y, pos.x, pos.y, drawColor, brushSize, true);
    lastPos.current = pos;
  };
      ...
      <div className="flex items-center justify-between bg-white/5 backdrop-blur-md p-6 rounded-[2rem] border border-white/10 shadow-2xl">
        <div className="flex flex-col gap-1">
          ...
          <div className="text-4xl font-black italic tracking-tighter text-white uppercase flex gap-2">
            {isDrawer ? secretWord : wordHint?.split(' ').map((char, i) => (
              <span key={i} className={char === '_' ? 'border-b-4 border-primary/40 min-w-[24px] text-center' : ''}>
                {char === '_' ? '' : char}
              </span>
            ))}
          </div>
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex flex-col items-end">
            ...
            <div className={`flex items-center gap-2 px-4 py-2 rounded-xl border-2 transition-colors ${
              timeLeft <= 10 ? 'border-red-500/50 bg-red-500/10 text-red-500' : 'border-primary/30 bg-primary/10 text-primary'
            }`}>
        ...
        <div className="relative group bg-white rounded-[2.5rem] shadow-2xl overflow-hidden border-8 border-white/5">
          {status === 'round_ended' && (
            <div className="absolute inset-0 bg-dark/90 backdrop-blur-md z-30 flex flex-col items-center justify-center p-8 text-center animate-in fade-in duration-500">
              <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="glass p-12 rounded-[3rem] border-primary/30 shadow-primary/20">
                <h2 className="text-5xl font-black italic uppercase tracking-tighter text-primary mb-4">ROUND OVER</h2>
                <p className="text-xl font-bold text-gray-400 mb-2 uppercase tracking-widest">The word was</p>
                <p className="text-4xl font-black text-white mb-10 bg-white/5 py-4 px-8 rounded-2xl border border-white/10 uppercase tracking-[0.2em]">{secretWord}</p>
                {isDrawer && (
                  <button onClick={handleStartNextRound} className="bg-primary hover:bg-primary-hover text-dark font-black py-5 px-12 rounded-2xl transition-all active:scale-95 shadow-lg shadow-primary/30 uppercase tracking-widest text-lg">
                    NEXT ROUND
                  </button>
                )}
                {!isDrawer && <p className="text-xs text-gray-500 font-bold uppercase tracking-[0.3em] animate-pulse">Waiting for drawer...</p>}
              </motion.div>
            </div>
          )}
          ...
          {isDrawer && status === GameStatus.PLAYING && (
            <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-4 animate-in slide-in-from-bottom duration-500">
              <div className="glass p-3 rounded-[2rem] flex items-center gap-4 border-white/10 shadow-2xl">
                ...
                <button onClick={clearCanvas} className="p-3 bg-red-500/10 hover:bg-red-500/20 text-red-500 rounded-xl transition-all border border-red-500/20 group">
              ...
              <div className="bg-dark/80 backdrop-blur-md px-4 py-2 rounded-full text-[10px] font-black text-gray-400 uppercase tracking-widest border border-white/5 shadow-xl">
                Drawing Tool Active: <span className="text-primary">{activeTool.toUpperCase()}</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col bg-white/5 backdrop-blur-md rounded-[2.5rem] border border-white/10 overflow-hidden shadow-2xl">
          <div className="p-6 border-b border-white/5 bg-white/5 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-xs font-black uppercase tracking-widest text-white">LIVE FEED</h3>
                <p className="text-[8px] font-bold text-gray-500 uppercase tracking-widest">Submit your guesses</p>
              </div>
            </div>
            {hasGuessed && (
          ...
          <form onSubmit={handleGuess} className="p-6 bg-white/5 border-t border-white/10 flex gap-3">
            <input
              type="text"
              value={guess}
              onChange={e => setGuess(e.target.value)}
              placeholder={(isDrawer || hasGuessed || status !== GameStatus.PLAYING) ? "WAITING..." : "TYPE GUESS..."}
            ...
            <button 
              type="submit" 
              disabled={isDrawer || hasGuessed || !guess.trim() || status !== GameStatus.PLAYING} 
